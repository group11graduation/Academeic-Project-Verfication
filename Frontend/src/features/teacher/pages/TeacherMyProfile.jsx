    ...
    return (
        <div className="mx-auto max-w-3xl space-y-4 antialiased [font-family:var(--sv-font-sans)]">
            <button
                type="button"
                onClick={() => navigate(-1)}
                className="inline-flex items-center gap-1.5 text-sm font-bold text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
            >
            ...
            <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] shadow-sm">
                <div className="border-b border-[var(--border)] bg-gradient-to-r from-[#eef3ff] via-white to-[#eef3ff] px-5 py-6 dark:from-[#111827] dark:via-[#0F172A] dark:to-[#111827]">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-[var(--brand-primary)] text-xl font-bold text-white shadow-md">
                            {photoUrl ? (
                        ...
                        <div className="min-w-0">
                            <h1 className="text-xl font-bold text-[var(--text-primary)]">{profile.name || 'Teacher'}</h1>
                            ...
                            {profile.employeeId ? (
                                <p className="mt-1 text-xs font-mono font-bold text-[var(--text-secondary)]">ID {profile.employeeId}</p>
                            ) : null}
                        </div>
                    </div>
                </div>
                ...
                {Array.isArray(profile.skills) && profile.skills.length > 0 ? (
                    <div className="border-t border-[var(--border)] px-5 py-4">
                        <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)]">Skills</p>
                        <div className="flex flex-wrap gap-1.5">
                            {profile.skills.map((skill) => (
                                <span
                                    key={skill}
                                    className="rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-bold text-[var(--brand-primary)] dark:bg-blue-950/40 dark:text-blue-300"
                                >
                                    {skill}
                                </span>
                            ))}
                ...
                <div className="border-t border-[var(--border)] px-5 py-4">
                    <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)]">Assigned classes</p>
                    {(profile.classes || []).length === 0 ? (
                        <p className="text-sm font-medium text-[var(--text-secondary)]">No classes assigned yet.</p>
                    ) : (
                            ...
                            {profile.classes.map((cls) => (
                                ...
                                >
                                    <span className="font-bold text-[var(--text-primary)]">
                                        {cls.code}
                                        {cls.name ? (
                                            <span className="font-medium text-[var(--text-secondary)]"> - {cls.name}</span>
                                        ) : null}
                                    </span>
                                    <button
                                        type="button"
