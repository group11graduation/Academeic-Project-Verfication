...
const AdminClasses = () => {
    ...
    const getCategoryStyles = (category) => {
        switch (category) {
            case 'ACADEMIC':
                return { color: 'bg-gradient-to-br from-[#1e2b72] to-[#3B82F6]', icon: GraduationCap };
            case 'LAB BASED':
                return { color: 'bg-gradient-to-br from-[#1e2b72] to-[#2563EB]', icon: Beaker };
            case 'THEORY':
                return { color: 'bg-gradient-to-br from-[#3B82F6] to-[#60A5FA]', icon: BookOpen };
            case 'WORKSHOP':
                return { color: 'bg-[#1E293B]', icon: Code2 };
            case 'SEMINAR':
                return { color: 'bg-gradient-to-br from-[#1e2b72] to-[#0EA5E9]', icon: Calculator };
            default:
                return { color: 'bg-gradient-to-br from-slate-600 to-slate-800', icon: BookOpen };
        }
    };

    const handleDeleteClass = async (e, item) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        try {
            ...
            const res = await adminClassService.deleteClass(code);
            if (!res.success) throw new Error(res.message || 'Failed to delete class');
            setClasses((prev) =>
                prev.filter((c) => String(c.code).toUpperCase() !== String(code).toUpperCase())
            );
            await appSuccess(
                res.message ||
                    `Class deleted. ${res.data?.studentsUnassigned ?? 0} student(s) kept as unassigned.`
            );
        } catch (error) {
            await appError(
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 md:gap-4">
                        ...
                        return (
                            ...
                            >
                                <div className={`h-[88px] relative p-4 ${style.color} flex flex-col justify-end`}>
                                    ...
                                    <button
                                        type="button"
                                        onClick={(e) => handleDeleteClass(e, item)}
                                        disabled={isDeleting}
                                        title="Delete class"
                                        className="absolute top-2.5 right-2.5 z-20 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-black/25 text-white backdrop-blur-sm transition hover:bg-rose-600 disabled:opacity-60"
                                    >
                                        {isDeleting ? (
                                ...
                                <div className="p-3.5 flex flex-col flex-1">
                                    ...
                                    <div className="flex items-center gap-2.5 py-2 px-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800 mb-3 mt-auto">
                                        <div className="p-1.5 bg-white dark:bg-slate-700 rounded-md shadow-sm">
                                            <Users className="h-3.5 w-3.5 text-[#1e2b72]" />
                                        </div>
                                        <div>
                                    ...
                                    <div className="text-[12px] font-bold text-[#1e2b72] group-hover:underline">
                                        View Details
                                    </div>
                                </div>
                            </div>
                    ...
                    >
                        <div className="w-10 h-10 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center shadow-sm border border-slate-100 dark:border-slate-700 mb-2 group-hover:scale-105 transition-transform">
                            <Plus className="h-5 w-5 text-[#1e2b72]" />
                        </div>
