...
export async function injectPreviewLoginCredentials(
  ...
  const script = [
    ...
    "      tmp=\"${html}.svcreds\"",
    "      { printf '%s\\n' \"<script>(function(){var c=\$PAYLOAD;window.__SV_PREVIEW_CREDS__=c;if(c&&c.apiBase)window.__SV_API_BASE__=c.apiBase;if(c&&c.loginPath)window.__SV_LOGIN_API_PATH__=c.loginPath;function n(o){if(!o||typeof o!=='object'||o.success===false)return o;var d=o.data&&typeof o.data==='object'?o.data:{};var t=o.token||o.accessToken||o.access_token||d.token||d.accessToken||d.access_token;var u=o.user||d.user;if(!t&&!u)return o;var x=Object.assign({},o);if(t){x.token=t;x.accessToken=x.accessToken||t;x.access_token=x.access_token||t;}if(u)x.user=u;x.success=true;x.message=x.message||'Login successful';x.data=Object.assign({},d,t?{token:t}:{},u?{user:u}:{},{success:true});try{if(t){localStorage.setItem('token',t);localStorage.setItem('accessToken',t);sessionStorage.setItem('token',t);}if(u)localStorage.setItem('user',JSON.stringify(u));}catch(e){}return x;}var of=window.fetch;if(typeof of==='function'&&!window.__SV_LOGIN_NORMALIZE__){window.__SV_LOGIN_NORMALIZE__=true;window.fetch=function(i,init){var m=String((init&&init.method)||'GET').toUpperCase();var u=typeof i==='string'?i:(i&&i.url)||'';var p=of.call(this,i,init);if(m!=='POST'||!/\\/(api\\/)?(auth|users|user|v1\\/auth)?\\/?login\\/?/i.test(u))return p;return p.then(function(r){if(!r||r.status<200||r.status>=300)return r;return r.clone().json().then(function(b){var nb=n(b);if(nb===b&&b&&b.success===true)return r;return new Response(JSON.stringify(nb),{status:r.status,statusText:r.statusText,headers:{'Content-Type':'application/json'}});}).catch(function(){return r;});});};}})();</script>\"; cat \"$html\"; } > \"$tmp\" && mv -f \"$tmp\" \"$html\" || rm -f \"$tmp\"",
    '    done',
    '  fi',
    'done',
    'echo ok',
