...
const PREVIEW_SAFETY_TEMPLATE = path.join(
  path.dirname(fileURLToPath(import.meta.url)),
  '../../docker-templates/node-js/preview-safety.cjs'
);
...
function buildCorsFixModule() {
  try {
    return readFileSync(PREVIEW_SAFETY_TEMPLATE, 'utf8');
  } catch (err) {
    throw new Error(
      'preview-safety.cjs missing at ' + PREVIEW_SAFETY_TEMPLATE + ': ' + err.message
    );
  }
}
