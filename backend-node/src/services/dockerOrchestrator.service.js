...
async function stagePreviewBaseBuildDir(templateDirName) {
  ...
  if (needsNodePreviewScripts) {
    ...
    const loginFallbackSrc = path.join(sharedNodeDir, 'preview-login-fallback.js');
    if (fsSync.existsSync(loginFallbackSrc)) {
      const loginFallback = await fs.readFile(loginFallbackSrc, 'utf8');
      await fs.writeFile(
  ...
  if (templateDirName === 'php-apache' || templateDirName === 'laravel-react-mysql') {
    ...
    const laravelLoginSrc = path.join(templateDir, 'preview-sv-login.php');
...
async function runPreviewContainer({
  ...
  if (
    ...
    const overlayFiles = [
      ...
      ['preview-mongo-preload.cjs', '/preview-mongo-preload.cjs'],
